...
const api: ChatApi = {
  ...
  send: (text) => call(IPC.send, text),
  sendImage: (image) => call(IPC.sendImage, image),
  nudge: () => call(IPC.sendNudge),
  sendWink: (wink) => call(IPC.sendWink, wink),
  connect: (host, port) => call(IPC.connect, host, port),
  getSaved: () => call(IPC.getSaved),
  forget: (host, port) => call(IPC.forget, host, port),
  onMessage: (cb) => subscribe(IPC.message, cb),
  onImage: (cb) => subscribe(IPC.image, cb),
